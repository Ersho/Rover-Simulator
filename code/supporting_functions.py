...
def update_rover(Rover, data):
      ...
      #maxa =  (np.amax(Rover.nav_angles))
      #mina =  (np.amin(Rover.nav_angles))
      #average = np.average(Rover.nav_angles)
      ...
      'samples collected:', Rover.samples_collected, 'Yaw', Rover.yaw)
      #'nav angles', maxa, mina)
      ...
      if rock_world_pos[0].any():
            ...
            for idx in range(len(Rover.samples_pos[0])):
                  ...
                  if np.min(rock_sample_dists) < 3:
                        Rover.samples_arr.append([test_rock_x,test_rock_y])
                        samples_located += 1
